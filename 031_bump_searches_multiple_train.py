...
def train_vae(name_fold, name_weights , dictionary, w, ind_w, intermediate_dim, act_fun, latent_dim, kernel_max_norm,
              lr, epochs, weight_KL_loss, batch_size, range_mbb, range_mct2, cols,
...
#         val = np.concatenate((back_val[:,0:1],back_val[:,2:4]), axis = 1)
        ...
        train_df = train_df[((train_df['mlb1']>=0)&(train_df['mlb1']<1000))]
        train_df = train_df[((train_df['lep1Pt']>=0)&(train_df['lep1Pt']<1000))]
#         train = train_df[['met', 'mt', 'mct2','mlb1','lep1Pt', 'nJet30','nBJet30_MV2c10']].values
        train = train_df[cols].values
#         train = np.concatenate((back[:,0:1],back[:,2:4]), axis = 1)
        ...
        val_df = val_df[((val_df['mlb1']>=0)&(val_df['mlb1']<1000))]
        val_df = val_df[((val_df['lep1Pt']>=0)&(val_df['lep1Pt']<1000))]
#         val = val_df[['met', 'mt', 'mct2','mlb1','lep1Pt', 'nJet30','nBJet30_MV2c10']].values
        val = val_df[cols].values

        try:
            os.makedirs(model_dep_results_bump_multiple + sig_inj[1] + '/' +  name_fold + '/' + name_weights)
        except:
            pass

        with open(model_dep_results_bump_multiple + sig_inj[1] + '/' + name_fold +  '/' + name_weights +
...
#                                             val
            ...
            callbacks = [TerminateOnNaN(),
                        ModelCheckpoint(model_results_bump_multiple + '{}/{}/vae_{}.h5'.format(name_fold, name_weights,
                                                                                                  num_model),
                                            monitor='val_loss',
                            ...
                            ReduceLROnPlateau(monitor='val_loss',
                                              ...
                                              min_lr=9e-8,
                                              verbose=1),
                        SaveAutoencoder(model_results_bump_multiple +'{}/{}/autoencoder_{}.h5'.format(name_fold, name_weights,
                                                                                                          num_model),
                                            monitor='val_loss',
                                            mode='auto', save_best_only=True,verbose=1,
                                            period=1)
            ...
            # or save to csv:
            hist_csv_file = model_results_bump_multiple +'{}/{}/history_{}.csv'.format(name_fold, name_weights, num_model)
...
#                                                                                                                     , name, num_model),
                        ModelCheckpoint(model_dep_results_bump_multiple + sig_inj[1] + '/{}/{}/vae_{}.h5'.format(name_fold,name_weights,
                                                                                                                num_model),
                                            monitor='val_loss',
                                            mode='auto', save_best_only=True,verbose=1,
                                            period=1),
                            ...
                            ReduceLROnPlateau(monitor='val_loss',
                                              ...
                                              min_lr=9e-8,
                                              verbose=1),
                        SaveAutoencoder(model_dep_results_bump_multiple + sig_inj[1] +'/{}/{}/autoencoder_{}.h5'.format(name_fold,
            ...
            # or save to csv:
            hist_csv_file = model_dep_results_bump_multiple + sig_inj[1] +'/{}/{}/history_{}.csv'.format(name_fold, name_weights, num_model)
            with open(hist_csv_file, mode='w') as f:
                hist_df.to_csv(f)
...
#     name = 'bump_feat_{}'.format('_'.join([str(x) for x in weights if x != 0]))
    ...
    train_vae(name_fold=name_fold, name_weights = name_weights, dictionary=components_dict, w=weights, ind_w = individual_weights,  intermediate_dim=50, act_fun='relu',latent_dim=latent_dim,kernel_max_norm=500, lr=0.003,epochs=2000, weight_KL_loss=0.6, 
              batch_size=200, range_mbb = range_mbb, range_mct2 = range_mct2,
              cols = cols, num_train=5, sig_inj = [True, '750_250'])
                                                    #[True, '187_12']
