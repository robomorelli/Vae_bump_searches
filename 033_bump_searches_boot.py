...
def train_vae(name_fold, name_weights , dictionary, w, ind_w, intermediate_dim, act_fun, latent_dim, kernel_max_norm,
              lr, epochs, weight_KL_loss, batch_size, cols,
              num_train=15, shift = 0, name = ''):
...
# #         train = np.concatenate((back[:,0:1],back[:,2:4]), axis = 1)
# #         val = np.concatenate((back_val[:,0:1],back_val[:,2:4]), axis = 1)

#         print('before cut {}'.format(len(back)))
#         print(cols)
...
#         try:
#             os.makedirs(model_results_bump_multiple + name_fold +  '/' + name_weights)
#         except:
#             pass

#         with open(model_results_bump_multiple +  name_fold +  '/' + name_weights + '/' + 'comps_dict.pickle', 'wb') as handle:
...
#         mod_ind = False
    ...
    for num_model in range(shift, num_train):
        print(num_model)

        back = np.load(train_val_test_mod_dep_boot_30_30_40 + 'bck_sig_{}_30_30_40/bck_sig_{}_boot_{}/background_sig_train_{}_boot_{}.npy'.format(name, name,num_model,name,num_model))
        back_val = np.load(train_val_test_mod_dep_boot_30_30_40 + 'bck_sig_{}_30_30_40/bck_sig_{}_boot_{}/background_sig_val_{}_boot_{}.npy'.format(name, name,num_model,name,num_model))


        print("\x1b[31m\" MODEL DEPENDENT on {}""\x1b[0m".format(sig))

        #
        # back = np.load(train_val_test_mod_dep + 'bck_sig_{}_30_30_40/background_sig_train_{}.npy'.format(sig,sig))
        # back_val = np.load(train_val_test_mod_dep + 'bck_sig_{}_30_30_40/background_sig_val_{}.npy'.format(sig,sig))
        #remove weights from the train-val data:
#         train = np.concatenate((back[:,0:1],back[:,2:4]), axis = 1)
#         val = np.concatenate((back_val[:,0:1],back_val[:,2:4]), axis = 1)
        ...
        try:
            os.makedirs(model_dep_results_boot + sig_inj + '/' +  name_fold + '/' + name_weights)
        except:
            pass

        with open(model_dep_results_boot + sig_inj + '/' + name_fold +  '/' + name_weights +
...
#                                             val
            ...
            callbacks = [TerminateOnNaN(),
                        ModelCheckpoint(model_dep_results_boot + '{}/{}/vae_{}.h5'.format(name_fold, name_weights,
                                                                                                  num_model),
                                            monitor='val_loss',
                            ...
                            ReduceLROnPlateau(monitor='val_loss',
                                              ...
                                              min_lr=9e-8,
                                              verbose=1),
                        SaveAutoencoder(model_dep_results_boot +'{}/{}/autoencoder_{}.h5'.format(name_fold, name_weights,
                                                                                                          num_model),
                                            monitor='val_loss',
                                            mode='auto', save_best_only=True,verbose=1,
                                            period=1)
            ...
            # or save to csv:
            hist_csv_file = model_dep_results_boot +'{}/{}/history_{}.csv'.format(name_fold, name_weights, num_model)
...
#                                                                                                                     , name, num_model),
                        ModelCheckpoint(model_dep_results_boot + sig_inj + '/{}/{}/vae_{}.h5'.format(name_fold,name_weights,
                                                                                                                num_model),
                                            monitor='val_loss',
                                            mode='auto', save_best_only=True,verbose=1,
                                            period=1),
                            ...
                            ReduceLROnPlateau(monitor='val_loss',
                                              ...
                                              min_lr=9e-8,
                                              verbose=1),
                        SaveAutoencoder(model_dep_results_boot + sig_inj +'/{}/{}/autoencoder_{}.h5'.format(name_fold,
            ...
            # or save to csv:
            hist_csv_file = model_dep_results_boot + sig_inj +'/{}/{}/history_{}.csv'.format(name_fold, name_weights, num_model)
            with open(hist_csv_file, mode='w') as f:
                hist_df.to_csv(f)
...
#     name = 'bump_feat_{}'.format('_'.join([str(x) for x in weights if x != 0]))


    names = ['550_300']
     #,'500_100',
    #'550_300','550_50','600_50',
    #names = ['650_300','700_50','750_250','800_400','900_50']
    
    name_fold = '{}_ft_{}_add_cuts'.format('_'.join([str(x) for x in selected_idx]),original_dim)
    name_weights = 'w_{}'.format('_'.join([str(x) for x in weights if x != 0]))

    for name in names:

        try:
            os.makedirs(model_dep_results_boot + name + '/' + name_fold + '/' + name_weights)
        except:
            pass

        shift = len(os.listdir(model_dep_results_boot + name + '/' + name_fold + '/' + name_weights))//3
    
        print(shift)

        if shift >= 15:
            print('{} completed moving to {}'.format(name, names[names.index(name)+1]))
    ...
    #     range_mct2 = [100,1000]
        ...
        train_vae(name_fold=name_fold, name_weights = name_weights, dictionary=components_dict,
         w=weights, ind_w = individual_weights,  intermediate_dim=50, act_fun='relu',latent_dim=latent_dim,kernel_max_norm=500, lr=0.003,epochs=2000,
          weight_KL_loss=0.6,
                  batch_size=200, cols = cols, num_train=15, shift = shift, name=name)
                                                        #[True, '187_12']
