...
def cutflow_bkg(data_folder, train_sample_name, depth):
...
#         batch = batch[batch['nBJet30_MV2c10']==2]
        ...
        if depth == 'preselection':

            print('preselection cutflow o cuts on mbb and mct2')
            
        else:

            print('preselection')
            
#            batch = batch[((batch['mbb']>=0)&(batch['mbb']<=300))]
...
#            print('after mct2 {}'.format(len(batch)))
        ...
        if len(batch) > 0:
            ...
            np.save(splitted_numpy_bkg + depth + '/background_{}_{}.npy'.format(name,ix), batch_fin.values)


    return


def cutflow_sig(data_folder, train_sample_name, depth):
...
#            print('after mct2 {}'.format(len(batch)))
        ...
        if len(batch) > 0:
            ...
            np.save(numpy_sig + depth + '/{}_{}.npy'.format(name,ix), batch_fin.values)
...
def concatenate_file(input_data_folder, output_data_folder, data_kind):
    ...
    # for signal it will give an erro

    return


def train_val_test_split(input_data_folder, output_data_folder, proportions):
    ...
    if len(proportions) == 2:
        ...
        X_train, X_val_test, _, _ = train_test_split(back_tot, y, test_size=proportions[1]+proportions[2], random_state=42)
